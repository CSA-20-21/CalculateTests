...
class CalculateTester1_1 {
	...
	void testToMixedNum() {
		...
		assertEquals(Calculate.toMixedNum(8, 0), "NaN");
		assertEquals(Calculate.toMixedNum(0, 8), "0");
		assertEquals(Calculate.toMixedNum(8, 4), "2");
	}
	...
	void testFOIL() {
		...
		assertEquals(Calculate.foil(0, 5, 0, -8, "t"), "-40");
		assertEquals(Calculate.foil(6, -11, -4, 7, "b"), "-24b^2 + 86b - 77");
	...
	void testDiscriminent() {
		...
		assertEquals(String.format("%.4f",Calculate.discriminant(-3.2, 0.0, 5.1)), String.format("%.4f",65.28));
		assertEquals(String.format("%.4f",Calculate.discriminant(0.0, 4.1, -3.4)), String.format("%.4f",16.81));
		assertEquals(String.format("%.4f",Calculate.discriminant(1.0, 2.0, 3.0)), String.format("%.4f",-8.0));
	}
